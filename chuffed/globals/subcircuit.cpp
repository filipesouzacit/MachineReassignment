...
public:
    ...
    void wakeup(int i, int c) {
        if (c & EVENT_F && x[i].getVal() != i) new_fixed.push(i);    // only put in new_fixed if it's in the circuit
        pushInQueue();
    }
    ...
    void explainAcantreachB(Clause *reason, int reasonIndex, vec<int> A, vec<int> B, int a1=-1, int b1=-1)
    {
    ...
    bool exploreSubtree(int thisNode, int startPrevSubtree, int endPrevSubtree,
     int *backfrom, int *backto, int *numback)
    {
		...
		// subtree to this node, as long as something inside the child has to be in.
                if(pruneWithin && isFirstChild && lowlink[child] == index[thisNode])
                {
                    ...
                    // The reason is that no node in the child's subtree reaches a node outside the subtree that isn't the parent, and at least one node in the child's subtree is in the circuit
    ...
    bool propagateSCC(int root) {
        ...
        for (typename IntView<U>::iterator i = x[root].begin(); i != x[root].end(); ++i) 
        {
		    ...
		    // previous one and both of these groups has
			    ...
			    // previous one or unexplored ones.
                            ...
                            explainAcantreachB(r, 3 + size1 + size2, thisSubtree, prev_later, thisSubtree[0], prev_later[0]);
                ...
                // When a new subtree has been explored, update the prev and earlier vectors (only necessary if explaining)
        ...
        {    
            ...
            // First find the nodes in the last subtree (and disconnected nodes) and those in the earlier ones
     ...
     /*   else if(selectionMethod == 3)
        ...
        else if(selectionMethod == 3)
        {
            ...
            for(int i = 0; i < options.size(); i++)
            {
                // XXX [AS] Replaced 'sat.level' by 'sat.trailpos', because it was replaced in rev. 441
                // Maybe it shoud be replaced by sat.getLevel(.)?
                if(sat.trailpos[var(x[options[0]].getLit(options[0],1))] != sat.trailpos[var(x[options[0]].getLit(options[0],0))])
                //if(sat.getLevel(var(x[options[0]].getLit(options[0],1))) != sat.getLevel(var(x[options[0]].getLit(options[0],0))))
...
/*
void pathWithMandatoryNodes(vec<IntVar*>& _x, int _startIndex, int _endIndex, vec<int>& _mandatoryIndices,
        bool _check, bool _prevent, bool _scc, bool _pruneRoot, 
        bool _pruneSkip, bool _fixReq, bool _generaliseScc) {
